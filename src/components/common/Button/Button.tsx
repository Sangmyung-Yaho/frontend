...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    <button
      ...
      type={type}
      disabled={disabled}
      className={`inline-flex h-14 items-center justify-center rounded-xl px-6 text-headline transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-main-500 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-300 ${variantClasses[variant]} ${fullWidth ? 'w-full' : ''} ${className}`}
      {...props}
    >
      {children}
    </button>
